...
## KEGG downloaded from Molecular Signatures Database 20220831
KEGGpath <- "/home/kaipu/microbiome_TCGA_miRNA/KEGG/c2.cp.kegg.v7.5.1.entrez.gmt"

KEGG <- gmtPathways(KEGGpath)
head(KEGG)
class(KEGG)
length(KEGG)
...
for (i in 1:length(taxaLevelList)) {
  ...
  for (i in 1:nrow(colorCode31)) {
    ...
    dir.create(file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "KEGG"), recursive=T, showWarnings=F)
    ...
    print("Start KEGG analysis by FGSEA...")
    j <- 1
    ...
    for (j in 1:ncol(bacteriaGeneSCCzTransform)) {
      ...
      KEGGresult <- fgsea(pathways=KEGG,
                          stats=preranks, 
                          nperm=1000, minSize=15, maxSize=500)
      
      KEGGresultPadjSort <- as.data.frame(KEGGresult[order(KEGGresult$padj, decreasing=F), ])
      head(KEGGresultPadjSort)
      dim(KEGGresultPadjSort)
      
      ## leadingEdge is list => convert to string and save in Gene column, and save it
      KEGGresultPadjSort$EntrezID <- NA
      KEGGresultPadjSort$GeneSymbol <- NA
      k <- 1
      for (k in 1:nrow(KEGGresultPadjSort)) {
        leadingEdgeGene <- unlist(KEGGresultPadjSort[k, 8])
        KEGGresultPadjSort$EntrezID[k] <- paste(leadingEdgeGene, collapse=",")
        
        entrez2symbolIndex <- match(leadingEdgeGene, geneInfoMerge[, 2])
        length(entrez2symbolIndex)
        
        KEGGresultPadjSort$GeneSymbol[k] <- paste(geneInfoMerge[entrez2symbolIndex, 4], collapse=",")
      }
      
      dir.create(file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "KEGG", bacteriaName), recursive=T, showWarnings=F)
      KEGGresultPadjSortOutput <- file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "KEGG", bacteriaName, "KEGG.txt")
      write.table(KEGGresultPadjSort[, -8], file=KEGGresultPadjSortOutput, sep="\t", col.names=T, row.names=F, quote=F)
      
      collapsedPathways <- collapsePathways(KEGGresult[order(pval)][padj < 0.05], 
                                            KEGG, preranks)
      length(collapsedPathways$mainPathways)
      
      KEGGresultPadjSigSortOutput <- file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "KEGG", bacteriaName, "KEGG_main_adjp_sig.txt")
      write.table(KEGGresultPadjSort[which(KEGGresultPadjSort$pathway %in% collapsedPathways$mainPathways), -8],
                  file=KEGGresultPadjSigSortOutput, sep="\t", col.names=T, row.names=F, quote=F)
