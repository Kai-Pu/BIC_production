...
length(gmpr.size.factor8)
head(gmpr.size.factor8)
#table(names(gmpr.size.factor8)==colnames(data)[2:ncol(data)])


## select gmpr.size.factors which are all not NAs
data_gmpr <- t(t(as.matrix(data[, 2:ncol(data)]))/gmpr.size.factor8)
dim(data_gmpr)
...
aliquot_barcode_NAs <- names(gmpr.size.factor8)[which(is.na(gmpr.size.factor8))]
