...
## Reactome downloaded from Molecular Signatures Database 20220831
ReactomePath <- "/home/kaipu/microbiome_TCGA_miRNA/Reactome/c2.cp.reactome.v7.5.1.entrez.gmt"

Reactome <- gmtPathways(ReactomePath)
head(Reactome)
class(Reactome)
length(Reactome)
...
taxaLevelList <- c("Genus", "Family", "Class", "Order", "Phylum")
...
for (i in 1:length(taxaLevelList)) {
  ...
  for (i in 1:nrow(colorCode31)) {
    ...
    dir.create(file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "Reactome"), recursive=T, showWarnings=F)
    ...
    print("Start Reactome analysis by FGSEA...")
    j <- 1
    ...
    for (j in 1:ncol(bacteriaGeneSCCzTransform)) {
      ...
      ReactomeResult <- fgsea(pathways=Reactome,
                          stats=preranks, 
                          nperm=1000, minSize=15, maxSize=500)
      
      ReactomeResultPadjSort <- as.data.frame(ReactomeResult[order(ReactomeResult$padj, decreasing=F), ])
      head(ReactomeResultPadjSort)
      dim(ReactomeResultPadjSort)
      
      ## leadingEdge is list => convert to string and save in Gene column, and save it
      ReactomeResultPadjSort$EntrezID <- NA
      ReactomeResultPadjSort$GeneSymbol <- NA
      k <- 1
      for (k in 1:nrow(ReactomeResultPadjSort)) {
        leadingEdgeGene <- unlist(ReactomeResultPadjSort[k, 8])
        ReactomeResultPadjSort$EntrezID[k] <- paste(leadingEdgeGene, collapse=",")
        
        entrez2symbolIndex <- match(leadingEdgeGene, geneInfoMerge[, 2])
        length(entrez2symbolIndex)
        
        ReactomeResultPadjSort$GeneSymbol[k] <- paste(geneInfoMerge[entrez2symbolIndex, 4], collapse=",")
      }
      
      dir.create(file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "Reactome", bacteriaName), recursive=T, showWarnings=F)
      ReactomeResultPadjSortOutput <- file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "Reactome", bacteriaName, "Reactome.txt")
      write.table(ReactomeResultPadjSort[, -8], file=ReactomeResultPadjSortOutput, sep="\t", col.names=T, row.names=F, quote=F)
      
      collapsedPathways <- collapsePathways(ReactomeResult[order(pval)][padj < 0.05], 
                                            Reactome, preranks)
      length(collapsedPathways$mainPathways)
      
      ReactomeResultPadjSigSortOutput <- file.path(getwd(), projectName, taxaLevel, "BacteriaGeneCorr", samplePercentLabel, "Reactome", bacteriaName, "Reactome_main_adjp_sig.txt")
      write.table(ReactomeResultPadjSort[which(ReactomeResultPadjSort$pathway %in% collapsedPathways$mainPathways), -8],
                  file=ReactomeResultPadjSigSortOutput, sep="\t", col.names=T, row.names=F, quote=F)
