...
table(round(colSums(data_gmpr_relative_abundance), 10)==1)
...
## filter bacteria which are 0 expression in all samples
row_sum <- rowSums(data_gmpr>0)
hist(row_sum)
row_sum[which(row_sum==0)]
dim(data_gmpr_df)
dim(data_gmpr_df[-which(row_sum==0), ])

if (length(row_sum[which(row_sum==0)])>0) {
  write.table(data_gmpr_df[-which(row_sum==0), ], file=output_gmpr_norm_count, sep="\t",
...
dim(data_gmpr_relative_abundance_df[-which(row_sum==0), ])
if (length(row_sum[which(row_sum==0)])>0) {
  write.table(data_gmpr_relative_abundance_df[-which(row_sum==0), ], file=output_gmpr_norm_relative_abundance, sep="\t",
            row.names=F, col.names=T, quote=F)
} else {
