...
## manifest file here is created by retrivedPatientInfoFromUUID.py!!! (columns are different from TCGA gdc manifest)
...
with open(project_manifest_path, 'r') as f:
	for line in f:
		...
		output_all = "TCGA-" + project_name + "_patient_sample_aliquot_barcode_uuid_sample_type_all.txt"		
		...
		output_list_all.append(output_all)
...
for i in range(len(project_name_list)):
	...
	with open(manifest_path_list[i], 'r') as f:
		for line in f:
			line = line.rstrip().split("\t")
			if line[0] == "cases.0.samples.0.portions.0.analytes.0.aliquots.0.aliquot_id":
				continue
			file_uuid = line[7]
			aliquot_barcode = line[12]
			...
			all_list.append([patient_barcode, sample_barcode, aliquot_barcode, file_uuid, is_ffpe, sample_type])
	...
	print("Numbers of all samples:", len(all_list))
...
fp = open(output_result_summary, "a")
