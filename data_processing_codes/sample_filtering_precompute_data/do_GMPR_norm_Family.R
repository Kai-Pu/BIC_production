...
## use gmpr.size.factor5 for normalization (intersect.no=6)
length(gmpr.size.factor5)
head(gmpr.size.factor5)




## select gmpr.size.factors which are all not NAs
data_gmpr <- t(t(as.matrix(data[, 2:ncol(data)]))/gmpr.size.factor5)
dim(data_gmpr)
...
aliquot_barcode_NAs <- names(gmpr.size.factor5)[which(is.na(gmpr.size.factor5))]
